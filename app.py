...
try:
    ...
    logger.info("成功初始化LINE Bot API")
except Exception as e:
    logger.error(f"初始化LINE Bot API失敗: {e}")

# 創建Flask應用
app = Flask(__name__)

# 使用 Yahoo Finance API 替代 twstock
def get_stock_price(stock_id):
    try:
        ...
        url = f"https://query1.finance.yahoo.com/v8/finance/chart/{yahoo_id}"
        headers = {
        ...
        return mock_prices.get(yahoo_id, 0)
    except Exception as e:
...
def scheduled_tasks():
    while True:
        try:
            ...
            logger.info(f"定時任務執行完成: {current_time}")
        except Exception as e:
...
def start_scheduler_route():
    try:
        ...
        logger.info("定時任務已手動啟動")
        return '定時任務已啟動'
    except Exception as e:
        logger.error(f"啟動定時任務失敗: {e}")
        return f'啟動定時任務失敗: {e}', 500
...
def handle_message(event):
    ...
    if text == '投資組合':
        send_portfolio_status()
        line_bot_api.reply_message(event.reply_token, TextSendMessage(text="已發送投資組合狀態"))
